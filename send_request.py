...
# CSV READER INPUT goes here
def send_request_to_server(request_input):
        with urllib.request.urlopen("http://127.0.0.1:8001/getdata?time=" + request_input) as response:
                response = [line.decode('utf-8') for line in response.readlines()]
                reader = csv.DictReader(response, delimiter='|')

                epoch_time = int(request_input)
                request_input = time.strftime('%Y-%m-%dT%H-%M-%S', time.localtime(epoch_time))

                # Write server response to json file

                f = open(request_input + ".json", "w")
                f.write('{\n')
